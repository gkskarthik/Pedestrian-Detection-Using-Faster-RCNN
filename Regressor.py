...
def classification_train_model():
    ...
    model_class.add(keras.layers.Dense(4, activation='linear'))
...
def regressor(Xtrain, ytrain):
    """
        Runs the regressor for identifying the bounding boxes.
    """
    model_regressor = classification_train_model()
    model_regressor.compile(optimizer='adam', loss='mean_squared_error', metrics=['mse'])

    model_regressor.fit(Xtrain, ytrain, epochs=10, batch_size=50)

    # Importing the model to json file for future use
    model_json = model_regressor.to_json()
    with open("model_regress.json", "w") as json_file:
        json_file.write(model_json)
    model_regressor.save_weights("model_regress.h5")
